...
#creating new variable to remove the df or there will be column counting issues in code. 
for (i in 1:length(leagueSplit$season)){
  leagueSplit$SeasonStart<-leagueSplit$season$seasonStart[i]
}
leagueSplit<- leagueSplit[,-leagueSplit$season$seasonStart]
leagueSplit<- leagueSplit[,-leagueSplit$season$SeasonStop]
...
colnames(matchTeamAttHome)[9]<-"matchDate";
colnames(matchTeamAttHome)[18]<-"homeStatsTakenDate";
colnames(matchTeamAttHome)[19:26]<-c("homeBuildUpPlaySpeed", "homeBuildUpPlayPassing", "homeChanceCreationPassing", "homeChanceCreationCrossing", "homeChanceCreationShooting", "homeDefencePressure", "homeDefenceAggression", "homeDefenceTeamWidth");
...
matchTeamAttAway<- matchTeamAttAway[,-c(1:9,11:20,23:25,27,28,30,32,34,35,37,39,41:44)];
#renaming columns 
...
```{r}
#finding previous outcome of every match played for home team in match 

#removing hours from date variable
for(i in 1:length(leagueSplit$date)){
  leagueSplit$date[i]<-substr(leagueSplit$date[i],1,10)
}
#converting to date
leagueSplit$date<-as.Date(leagueSplit$date, format = "%Y-%m-%d")
leagueSplit[order(leagueSplit$date),]

for(i in 1:length(leagueSplit)){
  for(j in 1:length(leagueSplit$home_team_api_id)){
    if(leagueSplit$home_team_api_id[j] %in% leagueSplit$home_team_api_id[1:j-1]){
      games<-leagueSplit[leagueSplit$home_team_api_id[j]==leagueSplit$home_team_api_id[1:j-1],]
      leagueSplit$previousMatch[j]<-min(games$date)
    }
  }
}

min(leagueSplit[leagueSplit$home_team_api_id==leagueSplit$home_team_api_id,])
...
#using last season in set (2015-2016) as the testing dataset, while all matches before the 2015-2016 season are in traiing
test<-matchTeamAttHA[matchTeamAttHA$season$seasonStart==2015,]
train<-matchTeamAttHA[matchTeamAttHA$season$seasonStart<2015,]
...
matchTeamAttHA$homeWin<- as.factor(matchTeamAttHA$homeWin)
glm_fit = glm(homeWin ~ homeBuildUpPlaySpeed+ homeBuildUpPlayPassing+ homeChanceCreationPassing+ homeChanceCreationCrossing+ homeChanceCreationShooting+ homeDefencePressure+ homeDefenceAggression+ homeDefenceTeamWidth+awayBuildUpPlaySpeed+ awayBuildUpPlayPassing+awayChanceCreationPassing+ awayChanceCreationCrossing+ awayChanceCreationShooting+ awayDefencePressure+ awayDefenceAggression+ awayDefenceTeamWidth,
              data = matchTeamAttHA,
              family = binomial)
summary(glm_fit)
