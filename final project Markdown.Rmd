...
for(i in 1:length(leagueSplit)){
  if (leagueSplit$outcome[i] == "Win"){
    for(j in 1:length(leagueTable)){
      if(leagueSplit$team_long_name[i]==leagueTable$Var1[j]){
        leagueTable$win=+1
      }
    }
  }
  if(leagueSplit $outcome[i] =="Lose"){
    for(j in i:length(leagueTable)){
  ...
  if(leagueSplit$outcome[i] =="Tie"){
    for(j in i:length(leagueTable)){
      if(leagueSplit$team_long_name[i]==leagueTable$Var1){
        leagueTable$tie=+1
      }
    }
  }
